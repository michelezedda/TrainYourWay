import { storageGetAsync, storageSetAsync, storageRemoveAsync, storageGetSync, storagePrefillCache } from './storage'
...
export async function loadInjuryState(): Promise<InjuryState | null> {
  try {
    const raw = await storageGetAsync(STORAGE_KEY)
    if (raw) storagePrefillCache(STORAGE_KEY, raw)
    return raw ? (JSON.parse(raw) as InjuryState) : null
  } catch {
    return null
  }
}

export async function saveInjuryState(state: InjuryState): Promise<void> {
  const json = JSON.stringify(state)
  storagePrefillCache(STORAGE_KEY, json)
  await storageSetAsync(STORAGE_KEY, json)
}

export async function clearInjuryState(): Promise<void> {
  storagePrefillCache(STORAGE_KEY, null)
  await storageRemoveAsync(STORAGE_KEY)
}
